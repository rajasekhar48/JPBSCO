...
const stripePromise = loadStripe(
  "pk_test_51QGwg5HoPKSjYoYv6x96vXKnRZ0pDI7nFXtXQ81Fq20DEXLwVANdk0b6P4octuqRv9SJo9dJ1TgIDwehGreU36zv001C5Iey9V"
);
    ...
    await updateProfileAction(
      {
        ...
        memberShipEndDate: new Date(
          new Date().getFullYear() +
            (fetchCurrentPlanFromSessionStroage?.type === "basic" ? 1 :
             fetchCurrentPlanFromSessionStroage?.plan === "teams" ? 2 : 5),
          new Date().getMonth(),
          new Date().getDate()
        ),
      },
        ...
        <div className="container mx-auto p-0 space-y-8">
          <div className="grid grid-cols-1 gap-x-4 gap-y-8 md:grid-cols-2 lg:grid-cols-3">
            {membershipPlans.map((plan, index) => (
              <CommonCard
                key={plan.type} // Unique key added here
                icon={
